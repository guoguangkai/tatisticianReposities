...
public class CustomerService {
    ...
    public HashMap<String, List> getNameOutMap() {
        return nameOutMap;
    }

    public HashMap<String, List> getNeedsOutMap() {
        return needsOutMap;
    }

    public HashMap<String, List> getMobileOutMap() {
        return mobileOutMap;
    }

    public List<EmployeePojo> checkAllOrder(){
        List<EmployeePojo> list = checkAll();
   ...
   /**
    ...
    @SuppressWarnings("all")
    public List<EmployeePojo> checkAll() {
        List<Customer> customerList = new ArrayList<>();
        customerList = customerDao.getAbnormalCustomerCount();
    ...
    private Boolean privateCheckMobile(String mobile, String owner) {
        ...
        if (mobileNotNull.length() != 11 && isContainChinese && isSpecialChar) {
            if (ifNewList) {
                List<String> mobileInnerList = new ArrayList();
                mobileInnerList.add(mobileNotNull + "【手机号不符合规则】】");
                mobileOutMap.put(owner, mobileInnerList);
            } else {
                List<String> mobileInnerList = mobileOutMap.get(owner);
                mobileInnerList.add("【手机号为空】】");
